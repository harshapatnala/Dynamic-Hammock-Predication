...
void pipeline_t::retire(size_t& instret) {
   ...
   bool head_valid;
   bool completed, exception, load_viol, br_misp, val_misp, load, store, branch, amo, csr,deactivated;
   reg_t offending_PC;
   ...
   // FIX_ME #17a BEGIN
      head_valid = REN->precommit(completed,exception,load_viol,br_misp,val_misp,load,store,branch,amo,csr, offending_PC,deactivated);
   // FIX_ME #17a END
   ...
   if (head_valid && completed ) {    // AL head exists and completed
        ...
        if (!exception) {
           if (load || store) {
              ...
              exception = LSU.commit(load, amo, amo_success,deactivated);
              if (amo && store)
           ...
           if(deactivated) assert(exception == false);
           if (exception)
        ...
        if (!exception && !load_viol) {
           ...
           //
           //HP-- printf("Commiting Instruction - Deactivate Bit %d\n", deactivated);
           // FIX_ME #17b BEGIN
              REN->commit();
           // FIX_ME #17b END

           // If the committed instruction is a branch, signal the branch predictor to commit its oldest branch.
           if((PAY.buf[PAY.head].instruction_type != NORMAL) && (PAY.buf[PAY.head].inst.opcode() != OP_JAL)) assert(branch==false); 
           if (branch) {

              // TODO (ER): Change the branch predictor interface as follows: FetchUnit->commit().
             if(PAY.buf[PAY.head].is_hammock == false){
              
              //printf("FETCHUNIT COMMIT Calling PC %llx\n", PAY.buf[PAY.head].pc);
              if(PAY.buf[PAY.head].instruction_type == NORMAL)
           ...
           // Check results.
           if(!deactivated && (PAY.buf[PAY.head].instruction_type != CMOV)) {
            //HP--printf("In checker current PC - %llx\n", PAY.buf[PAY.head].pc);
            //printf("PC-%llx Instruction Type - %d Debug_index %d\n", PAY.buf[PAY.head].pc, PAY.buf[PAY.head].instruction_type, PAY.buf[PAY.head].db_index);
            //if(PAY.buf[PAY.head].instruction_type == THEN) printf("Type-%d Then Debug_index %d Dest_Phy_reg/Pred_tag %d\n", PAY.buf[PAY.head].is_hammock, PAY.buf[PAY.head].db_index, PAY.buf[PAY.head].predication_tag);
            checker();
           }
         //   else {
         //      if(PAY.buf[PAY.head].instruction_type == CMOV)
         //      //printf("Type-%d Instruction Type - %d Debug_index %d\n", PAY.buf[PAY.head].is_hammock, PAY.buf[PAY.head].instruction_type, PAY.buf[PAY.head].db_index);
         //   }
           if(!deactivated && PAY.buf[PAY.head].instruction_type != CMOV){
           ...
           if (!deactivated && (amo || csr || br_misp || val_misp)) {
